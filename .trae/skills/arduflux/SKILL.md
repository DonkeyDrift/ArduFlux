...
- 配置文件：`ArduFlux.json`（自动生成/更新）
- 配置模板：`ArduFlux.template.json`
- 测试：`tests/`
...
当 `ArduFlux.json` 存在时，`upload.ps1` 会优先读取其中的配置（板型/FQBN、串口、输出目录、监视器开关与波特率等），实现统一管理。
...
- 使用 `vsce package` 生成 `arduflux-*.vsix`
