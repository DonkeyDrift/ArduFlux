...
- 新增可选 WSL 编译后端，默认保持本地编译行为不变
- 在高级选项中加入 WSL 编译、项目同步、库同步等配置并持久化到 `ArduFlux.json`
- MCP 编译任务输出补充 backend、distro、workspace 与耗时等元数据
- 修复串口监视器启动前预复位导致启动日志丢失的问题
...
- **上传端口自动回退**：配置的 COM 口不匹配时，自动遍历所有候选端口尝试上传，成功后写入 `lastSuccessfulAddress`
- **端口成功值同步回写**：上传或 monitor 自动切换到正确端口后，**同时更新** `port.address` 和 `port.lastSuccessfulAddress`，下一次操作直接使用正确端口
- **monitor 独立模式自动重试**：单独启动串口监视器时，优先使用 `port.address`，连接失败自动枚举候选端口逐个尝试，全部失败返回明确错误
- **上传后 monitor 端口联动**：上传成功后直接使用上传成功的端口启动监视器，不再读取配置中的 `port.address`

### 测试

- 新增 3 个端口自动重试相关的单元测试，测试总数 **151** 个全部通过
